...
func createAndRunContainer(domain string, image string, instances int, mem int, envs []string, ports []string, cmd string, appName string) {
	...
	client.Println(nil, "ID", "IMAGE", "CREATED", "STATUS", "ARUKAS DOMAIN", "ENDPOINT")
	client.Println(nil, appSet.Container.ID, appSet.Container.ImageName, appSet.Container.CreatedAt.String(),
		appSet.Container.StatusText, appSet.Container.ArukasDomain, appSet.Container.Endpoint)
}
